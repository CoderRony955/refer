...
def updatepkg(name: str, path: str):
    """Update existing package path in referdb
    """
    try:
        ...
        for entry in data:
            if entry["name"] == Filter.word(name):
                entry["path"] = Filter.word(path)  # update path
                ...
                console.print(
                    f"[bold]Updated Path for Package [bold]{name}[/bold]\n[bold]New Path:[/bold] [green]{path}[/green]")
                return
