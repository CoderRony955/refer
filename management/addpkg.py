...
def addpkg(name: str, path: str):
    """Add package path in referdb
    """
    try:
        # check given path first
        if not Validate.path(path=path):
            console.print(
                f"[bold]{path}[/bold] [yellow][!] Path does not exist![/yellow]")
            return
        ...
        new_data = {
            "name": Filter.word(name),
            "path": Filter.word(path)
        }

        if any(pkg["name"] == name for pkg in data):
            console.print(
                f"[yellow]Name '{name}' already exists! Choose another one.[/yellow]")
            return
        ...
        console.print(
            f"[bold]New Record Added!\nName:[/bold] [green]{name}[/green]\n[bold]Package:[/bold] [green]{path}[/green]")
